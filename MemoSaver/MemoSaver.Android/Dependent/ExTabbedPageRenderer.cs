...
[assembly: ExportRenderer(typeof(ExTabbedPage), typeof(ExTabbedPageRenderer))]
namespace MemoSaver.Droid
{
    public class ExTabbedPageRenderer : TabbedPageRenderer
    {
        ...
        int lastItemId=-1;
        public ExTabbedPageRenderer(Context context) : base(context)
        {
        }
        ...
        protected override void OnLayout(bool changed, int l, int t, int r, int b)
        {
            ...
            if (bottomNavigationView != null)
            {
                ...
                for (int i = 0; i < Element.Children.Count; i++)
                {
                    ...
                    {
                        SetupBottomNavigationView(item);
                        SetTabItemTextColor(bottomNavMenuView.GetChildAt(item.ItemId) as BottomNavigationItemView, selectedColor);
                    }
                    else
                    {
                        SetTabItemTextColor(bottomNavMenuView.GetChildAt(item.ItemId) as BottomNavigationItemView, normalColor);
                    }
                }
            }
        }
        ...
        void SetupBottomNavigationView(IMenuItem item)
        {
            ...
            bottomLine.SetStroke(lineWidth, Xamarin.Forms.Color.FromHex("555555").ToAndroid());

            GradientDrawable bottomLine2 = new GradientDrawable();
            bottomLine2.SetShape(ShapeType.Line);
            bottomLine2.SetStroke(lineWidth, Xamarin.Forms.Color.FromHex("dddddd").ToAndroid());

            var layerDrawable = new LayerDrawable(new Drawable[] { bottomLine, bottomLine2 });
            layerDrawable.SetLayerInset(0, leftOffset, itemHeight, rightOffset, 0);
            layerDrawable.SetLayerInset(1, 0, -itemHeight, 0, 0);
